...
function _eval_symbolic(name::String, args::Vector{String})
    ...
    try
        ...
        elseif name == "DIFF"
            ...
            @variables x
            expr = eval(Meta.parse(args[1]))
            var_sym = eval(Meta.parse(args[2]))
            diff_result = Symbolics.derivative(expr, var_sym)
            return Dict("ok"=>true, "result"=>string(diff_result))
        elseif name == "SIMPLIFY"
            # Symbolic simplification
            expr = eval(Meta.parse(args[1]))
            simplified = Symbolics.simplify(expr)
            return Dict("ok"=>true, "result"=>string(simplified))
        end
    catch e
